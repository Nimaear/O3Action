...
local Bind = {
	init = function (self)
		...
		frame.texture:SetTexture(0, 0, 0, .25)
		frame:Hide()

		frame.Update = function(self, b, spellmacro)
			...
			if spellmacro=="SPELL" then
				...
				GameTooltip:SetScript("OnHide", function(self)
					...
					self:SetPoint("BOTTOM", frame, "TOP", 0, 1)
					self:AddLine(frame.button.name, 1, 1, 1)
					frame.button.bindings = {GetBindingKey(spellmacro.." "..frame.button.name)}
					if #frame.button.bindings == 0 then
						self:AddLine("No bindings set.", .6, .6, .6)
					else
						self:AddDoubleLine("Binding", "Key", .6, .6, .6, .6, .6, .6)
						for i = 1, #frame.button.bindings do
							self:AddDoubleLine(i, frame.button.bindings[i])
						end
					end
					self:Show()
			...
			elseif spellmacro=="MACRO" then
				...
				GameTooltip:SetPoint("BOTTOM", frame, "TOP", 0, 1)
				GameTooltip:AddLine(frame.button.name, 1, 1, 1)
				...
				frame.button.bindings = {GetBindingKey(spellmacro.." "..frame.button.name)}
					if #frame.button.bindings == 0 then
						...
						for i = 1, #frame.button.bindings do
							GameTooltip:AddDoubleLine("Binding"..i, frame.button.bindings[i], 1, 1, 1)
						end
					end
				GameTooltip:Show()
			elseif spellmacro=="STANCE" or spellmacro=="PET" then
				...
				GameTooltip:SetScript("OnHide", function(self)
					...
					self:SetPoint("BOTTOM", frame, "TOP", 0, 1)
					self:AddLine(frame.button.name, 1, 1, 1)
					frame.button.bindings = {GetBindingKey(frame.button.bindstring)}
					if #frame.button.bindings == 0 then
						self:AddLine("No bindings set.", .6, .6, .6)
					else
						self:AddDoubleLine("Binding", "Key", .6, .6, .6, .6, .6, .6)
		...
		frame.Listener = function(self, key)
			if key == "ESCAPE" then
		...
		frame.HideFrame = function(self)
			self:ClearAllPoints()
			self:Hide()
			GameTooltip:Hide()
		end
		frame.Activate = function(self)
			self.enabled = true
			self:RegisterEvent("PLAYER_REGEN_DISABLED")
		end
		frame.Deactivate = function(self, save)
			if save then
				SaveBindings(2)
				print("Saved keybindings.")
			else
				LoadBindings(2)
				print("Discarded keybindings.")
			end
			self.enabled = false
			self:HideFrame()
		...
		StaticPopupDialogs["KEYBIND_MODE"] = {
			text = "Hover your mouse over any actionbutton to keybind. Press the escape key to clear the current binding.",
			button1 = "Save bindings",
			button2 = "Discard bindings",
			OnAccept = function() frame:Deactivate(true) end,
			OnCancel = function() frame:Deactivate(false) end,
			timeout = 0,
			whileDead = 1,
			hideOnEscape = false
		}
		...
		local function register(val)
			if val.IsProtected and val.GetObjectType and val.GetScript and val:GetObjectType()=="CheckButton" and val:IsProtected() then
				...
				if script==button then
					val:HookScript("OnEnter", function(self) frame:Update(self) end)
				elseif script==pet then
					val:HookScript("OnEnter", function(self) frame:Update(self, "PET") end)
				end
			end
		end
		...
		for i=1,12 do
			...
			b:HookScript("OnEnter", function(self) frame:Update(self, "SPELL") end)
		end
		
		local function registerMacro()
			for i=1,36 do
				local b = _G["MacroButton"..i]
				b:HookScript("OnEnter", function(self) frame:Update(self, "MACRO") end)
			end
		...
		frame:SetScript("OnEvent", function(self) self:Deactivate(false) end)
		frame:SetScript("OnLeave", function(self) self:HideFrame() end)
		frame:SetScript("OnKeyUp", function(self, key) self:Listener(key) end)
		frame:SetScript("OnMouseUp", function(self, key) self:Listener(key) end)
		frame:SetScript("OnMouseWheel", function(self, delta) if delta>0 then self:Listener("MOUSEWHEELUP") else self:Listener("MOUSEWHEELDOWN") end end)
	end,
	slash = function (self)
		if InCombatLockdown() then 
			print("Blizzard won't let us bind keys in combat :(") 
			return 
		end
		self.frame:Activate()
	...
	new = function (self)
		local frame, macros = CreateFrame("Frame", "NimBind", UIParent), 0
		local this = {
			frame = frame
		}
